...
export default function EditDialog({ children, post }: { children: React.ReactNode, post: TypeBlogPost }) {
    ...
    const update = async (draft: boolean) => {
        const r = await EditBlogPost(id!, post.id, form.title, form.content, form.tags, form.image, draft);
        if (r?.data?.success) {
            qc.setQueryData(["blog", id], (old: any) => ({
                ...old,
                posts: old.posts.map((p: any) => p.id === post.id ? r.data.data : p),
            }))
            setIsOpen(false);
        }
        else {
